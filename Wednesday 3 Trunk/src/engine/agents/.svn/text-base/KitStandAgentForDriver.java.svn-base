...
public class KitStandAgentForDriver extends Agent implements KitStand {
	String name;
	KitRobot kr;
	public mykit kita,kitb;
	enum STATE {NO_ACTION,RECEIVE_KIT,RECEIVING_PARTS,DONE}
	PartsRobot pr;
	...
	public class mykit {
		...
		public mykit() {
			...
			candy = new ArrayList<GuiCandy>();
		}
	}
	
	public KitStandAgentForDriver(String n){
		name = new String(n);
		kita = new mykit();
		kitb = new mykit();
	}
	...
	public void msgGiveParts(ArrayList<AgentPart> p, AgentKit kit){
		...
		if(kita.agentkit == kit) {
			...
			for(AgentPart candy: p){
				...
				//kita.agentkit.guikit.fillKit(kita.candy);
			}
		...
		//guicheckingstation.DoRecieveParts(c);
		stateChanged();
	}
